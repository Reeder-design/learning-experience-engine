...
const responseSchema = {
  type: "object",
  additionalProperties: false,
  required: ["project", "changeSummary", "reviewNotes"],
  properties: {
...
Return one complete branching-scenario project, not prose.
...
function buildTaskText(payload) {
  ...
  return [
    ...
    "",
    "Return the complete replacement project plus a concise changeSummary and any reviewNotes."
  ].filter(Boolean).join("\n");
}
...
function mockResult(payload) {
  ...
  return {
    project,
    changeSummary: ["Mock AI mode produced a valid project for integration testing."],
